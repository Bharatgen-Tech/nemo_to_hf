...
gen_config = lambda args, tokenizer: f"""{{
    ...
    "routed_scaling_factor": {1 if args.moe_router_topk_scaling_factor is None else args.moe_router_topk_scaling_factor},
...
class DCPKeyMapper:
    quant_device = 'cpu'
    quant_device = 'cpu'
    force_bf16 = False  # ← ADD
    FP32_PRESERVE_KEYS = {   # ← ADD
        'mlp.gate.weight',
    ...
    def map_kv(self, key, value, args, mtp_as_extra_layer):
        if key.startswith('module.'):
            key = key[7:]  # Strip the 'module.' prefix
    ...
    def quant_if_needed(cls, key, value, args):
        if not args.fp8:
            if value is None:
                return [key]
            # Always preserve router weights as fp32
            if (
                key in cls.FP32_PRESERVE_KEYS
                and getattr(args, 'moe_router_dtype', None) == 'fp32'
            ):
                return {key: value.float()}
            # Cast everything else to bf16 if force_bf16
            if cls.force_bf16:
                return {key: value.to(torch.bfloat16)}
            return {key: value}
...
class SafetensorPart:
    def __init__(self, idx, total, keys, nbytes, converter):
        ...
        self.idx = idx
        self.force_bf16 = getattr(converter.args, 'force_bf16', False)  # ← ADD
    ...
    def rewrite(self):
        ...
        DCPKeyMapper.force_bf16 = getattr(self.args, 'force_bf16', False)  # ← ADD
...
class DCPConverter:
    def __init__(self, ckpt, dst, force_bf16=False, force_fp8=False, mtp_as_extra_layer=False, override_tokenizer_path=None):
        ...
        with open("model.yaml", "r") as f:
            self.args = types.SimpleNamespace(**yaml.safe_load(f)['config'])
        #if force_bf16:
        #    self.args.fp8 = None
        if force_fp8:
            self.args.fp8 = 'e4m3'
        self.args.force_bf16 = force_bf16  # ← ADD
